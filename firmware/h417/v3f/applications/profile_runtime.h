 ...
 *
 * Load order at boot: active user slot from profile_store -> factory
 * AKPK image compiled into this firmware -> (caller falls back to the
 * legacy default_profile table when nothing validates).
 */
...
/* Parse a validated package and install it as the active runtime. */
