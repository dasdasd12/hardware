...
#if CH585_ADC_PROBE_USE_RIGHT_KEYMAP
static const ch585_adc_probe_key_map_t *ch585_adc_probe_right_map_by_index(uint8_t index)
{
...
static const ch585_adc_probe_key_map_t *ch585_adc_probe_right_map_by_key(uint8_t key_id)
{
    uint8_t i;

    for (i = 0U; i < CH585_ADC_PROBE_RIGHT_KEY_COUNT; i++)
    {
        if (g_adc_probe_right_key_map[i].key_id == key_id)
        {
            return &g_adc_probe_right_key_map[i];
        }
    }
...
{
    return (ch585_adc_probe_right_map_by_key(key_id) != NULL) ? 1U : 0U;
}
#else
static uint8_t ch585_adc_probe_key_is_enabled(uint8_t key_id)
{
...
static uint8_t ch585_adc_probe_pick_key(uint8_t requested_key)
{
    ...
    if (key_id == CH585_ADC_PROBE_SCAN_ALL_KEY)
    {
        ...
        if (g_adc_probe_scan_cursor >= CH585_ADC_PROBE_RIGHT_KEY_COUNT)
        {
            g_adc_probe_scan_cursor = 0U;
        }
        map = ch585_adc_probe_right_map_by_index(g_adc_probe_scan_cursor);
    ...
    {
        key_id = g_adc_probe_right_key_map[0].key_id;
    }
#endif
...
static void ch585_adc_probe_apply_calibration(void)
{
    ...
    for (i = 0U; i < CH585_ADC_PROBE_RIGHT_KEY_COUNT; i++)
    {
        const ch585_adc_probe_key_map_t *map = &g_adc_probe_right_key_map[i];
        ch585_key_config_t *cfg = &g_key_config[map->key_id];
...
static int ch585_adc_probe_update_enabled_keys(void)
{
    ...
    for (i = 0U; i < CH585_ADC_PROBE_RIGHT_KEY_COUNT; i++)
    {
        uint8_t key_id = g_adc_probe_right_key_map[i].key_id;
