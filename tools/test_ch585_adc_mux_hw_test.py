...
ADC_DRIVER_H = os.path.join(ROOT, "firmware", "ch585", "applications", "ch585_ads7948_mux_acq.h")
ADC_DRIVER_C = os.path.join(ROOT, "firmware", "ch585", "applications", "ch585_ads7948_mux_acq.c")
...
class Ch585AdcMuxHwTestPolicy(unittest.TestCase):
    def test_ch585_adc_mux_hw_test_imports_fork_driver_code(self):
        ...
        self.assertIn("CH585_HALF_ROLE := $(HALF)", makefile)
        self.assertIn("ifeq ($(HALF),right)", makefile)
        self.assertIn("ch585_adc_mux_scan", makefile)
        self.assertIn("$(CH585_FIRMWARE_ROOT)/applications", makefile)
