...
class FirmwareMainMvpContract(unittest.TestCase):
    ...
    def test_h417_v3f_caps_lock_led_tracks_host_hid_output_report(self):
        board_init = read_text(
            "firmware", "h417", "v3f", "applications", "board_init.c"
        )
        main_c = read_text(
            "firmware", "h417", "v3f", "applications", "main.c"
        )
        usbhs_core = read_text(
            "firmware",
            "h417",
            "v3f",
            "drivers",
        ...
        for descriptor in (usbhs_descriptor, usbfs_descriptor):
            for token in (
                ...
                self.assertIn(token, descriptor)
        self.assertIn("DEF_USBD_REPORT_DESC_LEN     148", usbfs_descriptor_h)

        for core in (usbhs_core, usbfs_core):
            self.assertIn("HID_SET_REPORT", core)
            self.assertIn("hid_nkro_on_output_report", core)

        for wrapper in (usbhs_wrapper, usbfs_wrapper):
            self.assertIn("H417_HID_KEYBOARD_LED_CAPS_LOCK 0x02U", wrapper)
            self.assertIn("hid_nkro_caps_lock_on", wrapper)
            self.assertIn("s_keyboard_leds = 0U", wrapper)

        for token in (
            "V3F_CAPS_LOCK_LED_PORT  GPIOB",
            "V3F_CAPS_LOCK_LED_PIN   GPIO_Pin_7",
            "GPIO_ResetBits(V3F_CAPS_LOCK_LED_PORT, V3F_CAPS_LOCK_LED_PIN)",
            "GPIO_SetBits(V3F_CAPS_LOCK_LED_PORT, V3F_CAPS_LOCK_LED_PIN)",
        ):
            self.assertIn(token, board_init)
        self.assertIn("v3f_board_caps_lock_led_set", main_c)
        self.assertIn("v3f_usb_hid_caps_lock_on", main_c)
