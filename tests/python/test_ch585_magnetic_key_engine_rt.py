    ...
    def test_default_rt_triggers_on_300_per_mille_travel(self):
        source = textwrap.dedent(
            ...
            int main(void)
            {
                ...
                if (require_true(cfg.released_adc == 790U,
                                 "2.048 V released endpoint must be 790") != 0)
                {
                    return 1;
                }
                if (require_true(cfg.pressed_adc == 532U,
                                 "2.048 V pressed endpoint must be 532") != 0)
                {
                    return 1;
                }
                ...
                /* With released=790 and pressed=532, raw=764 is about
                 * 100/1000 travel. 688 stays below a 300/1000 move from
                 * that valley, while 686 crosses it. */
                if (update_one(&engine, 764U, 0U, "baseline must not press") != 0) return 1;
                if (update_one(&engine, 688U, 0U, "less than 300/1000 must not press") != 0) return 1;
                if (update_one(&engine, 686U, 1U, "300/1000 downward travel must press") != 0) return 1;
                if (update_one(&engine, 763U, 1U, "less than 300/1000 upward travel must stay down") != 0) return 1;
                if (update_one(&engine, 764U, 0U, "300/1000 upward travel must release") != 0) return 1;
                if (update_one(&engine, 688U, 0U, "second less-than-delta move must not press") != 0) return 1;
                if (update_one(&engine, 686U, 1U, "RT re-press must not require static press_pm") != 0) return 1;
