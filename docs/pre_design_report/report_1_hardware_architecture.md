...
> **硬件平台**: CH32H417QEU6 + CH585 + 2.4寸800x480 RGB屏（可竖向安装） + 64MB SDRAM + 外部QSPI Flash（容量待定，最小不低于1Gbit） + 磁轴矩阵  
...
## 调研目标
基于CH32H417QEU6 MCU构建一个键盘上的AI终端设备，集成2.4寸800x480 RGB屏幕（产品形态可竖向安装）、64MB SDRAM、外部QSPI Flash（容量待定，最小不低于1Gbit）、磁轴矩阵、CH585蓝牙/2.4G无线模块，运行操作系统并实现对主流AI Agent（Codex、Claude Code等）的监控与操作。

## 核心结论

**1. 算力充足**: CH32H417 V5F大核 CoreMark 2292@400MHz，5.73 CoreMark/MHz，超越STM32H7的Cortex-M7（2292 vs 2010），满足800x480 UI渲染+网络通信+AI Agent全场景需求。

**2. 推荐技术栈**: RT-Thread Standard（V5F）+ V3F裸机/RT-Thread Nano的AMP异构架构 + LVGL（图形）+ lwIP（网络）+ WebSocket（AI通信）+ 桥接服务器架构 + 自研磁轴固件 + WebHID配置软件。

**3. 键盘架构**: H417 USB3.0直接有线输出到PC（默认1K/2K/4K/8K，32KHz作为待开发验证的极限模式），CH585负责BLE5.4+2.4G无线（2.4G目标8KHz），自研固件不依赖QMK/VIA，磁轴引擎全量实现RT/DKS/SpeedTap/SOCD。

**4. 差异化优势**: 2.4寸全彩屏+三模磁轴键盘+AI Agent控制界面形成明确差异化假设，WebHID免安装配置软件降低使用门槛，USB3.0为后续32KHz有线回报率开发保留带宽余量。
...
CH32H417QEU6作为沁恒微电子推出的旗舰级RISC-V双核MCU，其硬件架构在MCU品类中呈现出一种罕见的"跨界"特征：它既保留了微控制器的实时响应特性，又通过乱序超标量CPU、USB3.0接口和64MB SDRAM（512Mbit）扩展能力，逼近应用处理器的性能边界。本章将从核心架构、图形显示、连接外设三个维度展开深度解析，并基于定量数据评估其在800x480 UI渲染、网络通信和AI Agent监控场景下的性能边界与瓶颈。
...
### 1.2.1 LTDC控制器：支持800x480@60fps

CH32H417集成的LCD-TFT显示控制器（LTDC, LCD-TFT Display Controller）最高支持XGA（1024×768）分辨率，配备2层独立显示层，支持Alpha混合、颜色键控和FRC抖动 [(Github)](https://github.com/openwch/ch32h417) 。目标屏KD024WVFPD102A的原生规格为800x480@60fps RGB并口，若产品采用竖向安装，应按800x480扫描时序驱动，由UI布局或软件旋转处理竖屏体验。

屏幕规格书给出的典型像素时钟为27MHz。以RGB565格式（每像素2字节）计算，显示刷新带宽约为27MHz × 2B = 54 MB/s。CH32H417的LTDC输出能力和像素时钟余量均覆盖该屏需求。双图层功能允许将静态背景与动态UI元素分离渲染，LTDC在硬件层面完成图层叠加，无需CPU介入 [(CSDN博客)](https://blog.csdn.net/weixin_42418754/article/details/151778738) 。

### 1.2.2 GPHA图形加速器：2D填充/BitBlt/Alpha混合

GPHA（Graphics Processing Hardware Accelerator）是CH32H417内置的2D图形硬件加速器。本地参考手册已给出寄存器描述，确认能力包括寄存器到内存填充、内存到内存拷贝、像素格式转换、Alpha混合、CLUT以及RGB565/RGB888/ARGB8888等多种像素格式。不应把90/180/270度旋转、缩放、RLE解码列为已确认硬件能力，除非后续从WCH示例或实测中得到证明。

GPHA的架构定位与STM32H7系列的Chrom-ART（即DMA2D）相似 [(weilnetz.de)](https://qemu.weilnetz.de/doc/6.0/system/target-riscv.html) 。基于同类硬件加速器的行业基准进行类比估算，优先把它用于全屏清屏、矩形填充、BitBlt、像素格式转换和Alpha混合。实际加速比仍需评估板实测；竖屏显示若需要旋转，应先准备LVGL软件路径或直接按竖向UI布局渲染，而不是依赖GPHA旋转。

### 1.2.3 FMC外接SDRAM：64MB扩展（512Mbit）

CH32H417的FMC（Flexible Memory Controller）控制器支持外接标准SDRAM，最高容量64MB（512Mbit） [(Github)](https://github.com/openwch/ch32h417) 。本地数据手册给出的SDRAM接口时钟上限为100MHz，数据位宽支持8/16/32-bit配置。以32-bit位宽@100MHz计算，理论峰值带宽为400 MB/s；若采用16-bit位宽，则理论峰值为200 MB/s。考虑SDRAM刷新、预充电、行列激活和LTDC持续读取等开销，实际可用带宽需在评估板上实测，不再按150MHz SDRAM时钟估算。

对于800x480 RGB565显示的帧缓冲需求，单帧需768KB，双帧缓冲需1.5MB [(CSDN博客)](https://blog.csdn.net/uran/article/details/155792428) 。这一容量已超出片上896KB SRAM的总容量，因此外接SDRAM并非"锦上添花"，而是实现流畅双缓冲显示的必需条件。64MB的SDRAM（512Mbit）容量意味着帧缓冲仅占用了不到3%的空间，剩余内存可容纳UI资源、网络缓存和轻量运行时，但仍不能按数百MB级缓存规划。
...
综合评估，CH32H417在CPU算力、图形处理、连接能力三个维度均满足键盘AI终端项目的需求。其5.73 CoreMark/MHz的V5F大核配合LTDC和GPHA基础2D能力，预估可在800x480分辨率下实现可用的UI渲染；USB3.0和百兆以太网的组合为AI Agent通信提供充裕的带宽余量；64MB SDRAM（512Mbit）扩展解决了帧缓冲和中等规模缓存需求。主要风险点集中在RISC-V工具链、GPHA实际吞吐、SDRAM板级信号完整性以及USB3.0 HID高回报率链路上，建议在项目启动初期即采购评估板进行实际验证测试。
...
CH32H417的硬件架构对RTOS提出了两项核心约束：其一，896KB片上SRAM与64MB外接SDRAM（512Mbit）的混合内存架构要求操作系统具备灵活的内存管理能力；其二，V5F（400MHz）与V3F（150MHz）是异构双核，更适合采用非对称多处理（AMP）分工，SMP仅作为后续实验选项。下表从七个关键维度对四款RTOS进行定量评估。
...
| 多核/AMP支持 | 同构SMP需额外适配 | 可作为V5F主OS，V3F走裸机/Nano AMP | QEMU验证为主 | 有SMP框架但WCH适配不足 |
...
图形子系统是AI终端与用户交互的核心通道。CH32H417驱动2.4寸800x480 RGB屏（产品可竖向安装使用）需要在有限的片上SRAM和较大带宽差异的内存层次结构之间取得平衡——896KB内部SRAM对64MB外接SDRAM（512Mbit）。本章从GUI框架选型、MVP终端方案、渲染优化策略及GPHA加速器四个维度展开分析，给出可落地的内存分配方案和性能预期。

### 3.1 LVGL详细评估

#### 3.1.1 400MHz RISC-V驱动800x480预估45-60FPS（中等复杂度UI）
...
LVGL在CH32H417上的性能预估最直接的对标对象是ESP32-P4——同为400MHz双核RISC-V MCU。实测数据显示，ESP32-P4驱动480x480分辨率（16bpp）在LVGL v9下可达约90 FPS，驱动1024x600分辨率仍可维持58-62 FPS  [(Quick Fix Surrey)](https://quickfixsurrey.ca/esp32-p4-vs-esp32-s3/) 。800x480分辨率（384,000像素）的像素总量介于480x480（230,400像素）与1024x600（614,400像素）之间，据此推算，CH32H417在中等复杂度UI场景下预估可达45-60 FPS。此外，CH32H417的CoreMark/MHz为5.73  [(Github)](https://github.com/openwch/ch32h417) ，显著高于ESP32-P4的3.16，单核整数性能更强，但ESP32-P4配备成熟的2D-PPA硬件加速器，CH32H417的GPHA吞吐和SDK支持仍需实测，因此上述预估需保留纯软件fallback。

#### 3.1.2 内存需求分析：双缓冲~1.5MB放SDRAM，内部SRAM保留给关键数据

对于800x480 RGB565显示配置，帧缓冲的内存开销需要精确计算。每个像素以2字节存储（RGB565），单帧缓冲大小为800 x 480 x 2 = 750KB。双缓冲策略下，前后帧缓冲合计1.5MB，需放置于外接SDRAM中。该配置约占64MB SDRAM（512Mbit）总量的2.3%，内存压力可控，但带宽规划不可忽视——SDRAM接口上限按100MHz核算，实际持续带宽需在目标PCB上实测，双缓冲交替读写时的Bank冲突也将影响吞吐。
...
色彩深度的选择本质上是视觉质量与内存带宽的权衡。在800x480@60Hz配置下，三种常见色彩格式的带宽需求差异显著：RGB565（16bpp）约消耗46.1MB/s，RGB888（24bpp）上升至69.1MB/s，ARGB8888（32bpp）则达到92.2MB/s。RGB565的65,536色在2.4寸屏幕上已能提供可接受的渐变平滑度，且相比RGB888节省33%带宽  [(Github)](https://github.com/stevenchadburrow/PICnes) 。鉴于CH32H417的SDRAM带宽预算中LTDC刷新会持续占用固定份额，选择RGB565可为UI渲染、GPHA操作及网络通信保留更充分的余量。
...
<table>
<caption><b>表3-2 帧缓冲策略对比（800x480 RGB565）</b></caption>
<thead style="background:#f0f0f0">
...
#### 3.4.1 GPHA潜力与风险：基础2D能力明确，性能和SDK仍需实测

GPHA（Graphics Processing Hardware Accelerator）是CH32H417集成的2D图形加速器。本地参考手册已提供寄存器章节，基础能力包括矩形填充、位图拷贝、像素格式转换和Alpha混合等，适合对标STM32的DMA2D（Chrom-ART）或ESP32-P4的2D-PPA（Pixel Processing Accelerator）。真正的不确定性不在于“是否有文档”，而在于SDK示例、总线仲裁、峰值吞吐、错误处理和LVGL draw unit适配工作量。以下基于同类加速器的实测数据，评估GPHA对渲染性能的理论贡献；旋转、缩放、RLE解码不列为已确认能力。
...
鉴于性能和SDK支持仍需实测，渲染架构必须采用分层抽象设计：底层为软件渲染器（纯CPU实现，确保功能可用），中间层为GPHA硬件加速单元（通过统一的draw unit接口挂接），上层为LVGL核心。若GPHA无法及时投入量产使用，软件渲染路径仍应能独立支撑MVP演示。建议优先联系WCH获取GPHA示例代码和应用笔记，同时在SDK中保留纯软件fallback的编译选项，确保开发进度不受阻塞。
...
- **磁轴模拟信号**：毫米级行程精度为待测试指标，目标支持可编程触发点和Rapid Trigger
...
**注意**：在本架构中，磁轴矩阵采集在H417上进行（通过MUX+ADC），CH585不直接连接键盘矩阵。CH585的角色是**无线modem + H417的USB HS外设**；有线USB到PC由H417负责。
...
|------|---------|---------|------|-----|---------|
| **本方案** | CH32H417+CH585 | 三模集成 | 2.4寸800x480 | 支持 | 是 |
...
**本方案的差异化优势**：
1. 大尺寸RGB屏幕（800x480）和三模连接形成明显差异化
2. 键盘事件可被本地OS消费（不只是HID设备）
...
| ADC分辨率 | 12-bit | 4096级ADC码；实际毫米精度取决于霍尔传感器、MUX、噪声、结构公差和校准算法，0.04mm级为待测试目标 |
| 全键无冲 | 支持 | 同时检测所有按键状态 |
| 延迟 | <2ms | 从按键到事件上报的总延迟 |
...
CH32H417的双核异构架构（QingKe V5F@400MHz + V3F@150MHz）与64MB SDRAM（512Mbit）、外部QSPI Flash（容量待定，最小不低于1Gbit）共同构成了一套独特的资源组合。如何在896KB片上SRAM与64MB外接SDRAM（512Mbit）之间建立层次化内存模型，如何在运算核与控制核之间划分职责边界，以及如何利用A/B双分区机制实现可靠远程升级——这些决策将直接决定终端的响应延迟与功能扩展能力。

### 5.1 双核任务分配

#### 5.1.1 V5F@400MHz：UI渲染 + AI Agent应用逻辑 + 文件系统

V5F作为乱序多发超标量处理器，配备32KB I-Cache和384KB零等待紧耦合内存（128KB ITCM + 256KB DTCM），CoreMark成绩2292分（5.73/MHz） [(CSDN博客)](https://blog.csdn.net/i7j8k9l/article/details/149688164) 。该核运行RT-Thread标准版，承载三大功能域：UI渲染引擎（LVGL图形库，以60FPS刷新800×480屏幕，双缓冲策略依赖SDRAM帧缓冲区）、AI Agent应用逻辑（WebSocket客户端JSON-RPC 2.0消息解析与状态机维护）、以及文件系统服务（通过RT-Thread DFS层挂载LittleFS，配置与日志的持久化读写）。
...
SDRAM采用固定预算而非数百MB级分区：帧缓冲与显示区（4MB）容纳LVGL双缓冲（800×480×2字节×2帧=1.5MB）及绘制缓冲；网络缓存区（8MB）分配给lwIP pbuf池、TCP窗口与WebSocket缓冲；文件系统缓存区（8MB）作为LittleFS与QSPI Flash间的读写缓冲；开源运行时区（8MB）预留MicroPython/Lua解释器；资源与用户数据热缓存区（约32MB）存储GUI资源、字体、日志窗口和AI Agent短期缓存。64MB SDRAM只能作为运行缓存，长期用户数据应落在外部QSPI Flash；Flash最终容量未定，当前只约束最小不低于1Gbit。
...
| 外接SDRAM | 网络缓存 | +4MB | 8MB | lwIP pbuf池/TCP窗口/DMA描述符/WebSocket缓冲 | 突发大容量缓冲 |
| 外接SDRAM | 文件缓存 | +12MB | 8MB | LittleFS读写缓存/热数据驻留 | 减少Flash擦写 |
| 外接SDRAM | 开源运行时 | +20MB | 8MB | MicroPython/Lua解释器执行区 | 脚本扩展支撑 |
| 外接SDRAM | 资源/用户热缓存 | +28MB | ~32MB | GUI资源/字体/Agent短期缓存 | 长期数据存入外部QSPI Flash |
...
#### 5.3.1 外部QSPI Flash分区方案（容量待定，最小1Gbit）

QSPI Flash容量尚未定型，当前约束为最小不低于1Gbit（128MB）。四线I/O@133MHz读取带宽按30-40MB/s估算，但写入仅约0.4MB/s（受页编程0.3-0.7ms/256字节限制） [(CSDN博客)](https://blog.csdn.net/weixin_35749440/article/details/156434531) 。"读快写慢"特性要求读取密集资源与写入频繁数据物理隔离。调查阶段不再按固定大容量规划，分区应随最终容量缩放：Bootloader预留512KB-1MB，Firmware A/B各4-8MB，LittleFS系统分区16-32MB用于配置、日志和OTA临时文件，GUI资源/字体/脚本包/用户数据使用剩余空间。若后续选用大于1Gbit的器件，可按资源包、日志保留周期和用户脚本需求扩展数据分区。

QSPI是否用于XIP执行需在CH32H417 QSPI章节和官方示例中进一步确认；在调查报告阶段，保守方案是把外部Flash定位为资源、日志、OTA镜像和大文件存储，关键代码仍从片内CodeFlash/ITCM/DTCM运行。若后续确认不能XIP，则固件或资源需由Bootloader加载到SRAM/SDRAM后使用。
...
- 2.4寸800x480屏幕可横向使用，也可竖向安装显示Agent状态列表+详情，尺寸合理
- 定价参考：$49-89是甜点区间
...
- **Dim02**: 官方BSP支持，最强GUI生态，V5F主OS+V3F实时协处理的AMP模式更适合该异构双核
...
- **Dim12**: GPHA可用于基础2D加速，实际加速比需评估板实测
- **结论**: LVGL完全可行，置信度**高**
